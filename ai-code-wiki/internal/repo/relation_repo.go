...
// ListByModule 查询某模块的上下游依赖关系。
// direction=out 查询源模块指向哪些模块；direction=in 查询哪些模块依赖源模块。
func (r *ModuleRelationRepo) ListByModule(module string, direction string) ([]*model.ModuleRelation, error) {
	var list []*model.ModuleRelation
	query := withNotDeleted(r.DB)
	if direction == "in" {
...
// GetByRelation 按 (源模块, 目标模块, 关系类型) 查询唯一关系。
func (r *ModuleRelationRepo) GetByRelation(sourceModule, targetModule string, relationType int8) (*model.ModuleRelation, error) {
...
// ListRelationsByModules 查询与给定模块集合相关的依赖关系（源或目标命中）。
// 业务规则：合并 AST 自动识别(source=1) 与 人工添加(source=2) 的关系，
// 查询时【不过滤 source 字段】，保证人工新增依赖不被丢弃。
func (r *ModuleRelationRepo) ListRelationsByModules(modules []string) ([]*model.ModuleRelation, error) {
	var list []*model.ModuleRelation
	query := withNotDeleted(r.DB)
	if len(modules) > 0 {
